...
export class DifficultyTable {
  ...
  set_score(songs, scores) {
    let table = this
    Object.entries(table.levels).forEach(([level_label, hashes]) =>
      hashes.forEach((hash) => {
        ...
        table.table_score[level_label][hash].init_song(
          songs.get_score(hash),
          hash
        )
  ...
  set_rival_score(scores) {
    ...
    Object.entries(table.levels).forEach(([level_label, hashes]) =>
      hashes.forEach((hash) => {
        ...
        table.table_score[level_label][hash].init_rival_score(
          scores.get_score(hash)
        )
      })
    )
    return table
   ...
   */
  get_filtered_score(filter) {
    return Object.values(this.table_score)
      .map(
        /**
